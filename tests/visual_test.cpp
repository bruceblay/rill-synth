...
int main() {
  ...
  // Contour ignores hits; all other families still answer different phrases.
  for(unsigned slot=0;slot<light::Painting::rotationCount;++slot) {
    unsigned family=light::Painting::familyAt(slot);
    a->seed(17); b->seed(17);
    while(a->visualFamily()!=family) { a->regenerate(); b->regenerate(); }
    ...
    if(family==light::Painting::Contour) assert(hash(*a)==hash(*b));
    else assert(hash(*a)!=hash(*b));
  }
  // Hit timing, pitch and strength must not affect any Contour frame.
  a->seed(17); b->seed(17);
  while(a->visualFamily()!=light::Painting::Contour){a->regenerate();b->regenerate();}
